...
export const displayClientInfo = (parent,data) => {
    parent.innerHTML += `
    <p class="booking__client-itom">Имя: ${data.fullName} </p>
    <p class="booking__client-itom">Телефон: ${data.phone} </p>
    <p class="booking__client-itom">Номер билета: ${data.ticketNumber} </p>
    ` 
}